...
def plt_timeseries(v,key,dim=0,downSample=1,lw=2,k_cut=0,q=2):
    """ fancy version of plt for timeseries plotting """
    d=downSample
    if key in ('wm','lm'):
        m,s2 = ('m','s2') if key=='wm' else ('mu','sig2')
        # ground truth
        yplt = np.exp(v['lam'][k_cut:-1:d,dim]) if key=='wm' else v['lam'][::d,dim]
        tspan = np.arange(0,len(yplt))*dt*d
        plt.plot(tspan,yplt,'k',linewidth=lw)
        # filter
        yplt, err = v[m][k_cut:-1:d,dim], v[s2][k_cut:-1:d,dim]**0.5            
        plt.fill_between(tspan,yplt-q*err,yplt+q*err,alpha=0.3,color='r')
        plt.plot(tspan,yplt,'r',linewidth=lw)
...
def plt_figs(fig,mp,lw=3,fontsize=18,plt_path='./',res_path='./'):
    ...
    if fig == 2:
        ...
        for r in ['Linear','Binary','RL']:
            # load
            v_ts = load_obj('fig_2_rule_{0}'.format(r),path=plt_path)            
            tau_ou = mp['tau_ou-dict'][r]
            k_cut = int(mp['cut_steps']*tau_ou/dt)            
            ...
            for i_dim in range(len(plt_dim)):
                ...
                plt_timeseries(v_ts,'wm',dim=i_dim,downSample=mp['down_sample']
                               ,lw=lw,k_cut=k_cut,q=2)
    ...
    if fig == 3 and my_args.i == -1: # MSE, not on cluster
        out = get_df(plt_path,contains_str='fig_3')
        y = 'MSE'        
    ...
    # fig 5 # robustness (keep gm const), not on cluster
        ...
        for plt_id in [1,2,3]: # hyperparameters
            ...
            out = get_df(plt_path,contains_str='fig_5{0}'.format(plt_id))
        ...
        # load
        out = get_df(plt_path,contains_str='fig_6')
        y = 'MSE'
    ...
    # fig 4: supp plots and fits
        ...
        outs = [load_obj(file[:-4],plt_path) for file in 
             sorted(os.listdir(plt_path)) if (file.endswith(".pkl") and 'fig_4' in file)] # and file.startswith("Supp")]
